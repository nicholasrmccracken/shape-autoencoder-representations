...
def draw_shape(shape, color):
    ...
    s  = random.uniform(0.4, 0.8) * IMG_SIZE   # “size” of the shape
    cx = random.uniform(0.3, 0.7) * IMG_SIZE   # center x
    cy = random.uniform(0.3, 0.7) * IMG_SIZE   # center y
    ...
    if shape == 'circle':
        ...
        d.ellipse(bounding_box, fill=color)
        ...
        r = s / 2.0
        base_angles = [0.0, 2.0 * math.pi / 3.0, 4.0 * math.pi / 3.0]
...
class ColorShapeDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        return img_tensor, shape_idx, color_idx


def build_transform(is_train: bool = True) -> transforms.Compose:
    ops = [
        transforms.Resize(IMG_SIZE),
        # Q4 Change: Remove color information to focus on structure using Grayscale
        transforms.Grayscale(num_output_channels=3),
    ]
    if is_train:
        # Q4 Change: Add strong rotation for shape invariance
        ops.append(transforms.RandomRotation(degrees=(-180, 180))) 
...
def build_dataloader(data_dir: str,
    ...
    """
        ...
        Args:
            ...
            is_train: Whether the DataLoader is for training (affects data augmentation).
    ...
    dataset = ColorShapeDataset(all_paths, transform=build_transform(is_train=is_train))
    loader = DataLoader(
        dataset,
...
def train_autoencoder(model: ConvAutoencoder, train_loader: DataLoader,
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch + 1}/{epochs} - train MSE loss: {average_loss:.6f}")
...
def collect_embeddings(model: ConvAutoencoder, loader: DataLoader,
    ...
    with torch.no_grad():
        ...
        for images, batch_shapes, batch_colors in loader:
            images = images.to(device)
            features['pixels'].append(flatten_feature(images).cpu())

            _, intermediates = model.encode(images, return_intermediate=True) 
            for i, intermediate in enumerate(intermediates):
                layer_name = f'layer_{i+1}'
                features[layer_name].append(flatten_feature(intermediate).cpu())

            shapes.append(batch_shapes)
...
def save_nearest_neighbor_grid_cross(
    ...
    r = F.normalize(ref_embeddings, dim=1)
    dist = torch.cdist(q, r)
    ...
    for q_idx in query_indices:
        ...
        _, nn_indices_tensor = torch.topk(dist[q_idx], k=neighbors, largest=False)
        nn_indices = nn_indices_tensor.cpu().numpy()
...
def plot_layerwise_nn(color_accs, shape_accs, out_path: str):
    ...
    num_layers = len(color_accs)
    x = list(range(num_layers))
    layer_labels = ['pixels'] + [f'layer_{i}' for i in range(1, num_layers)]
...
def evaluate(model: ConvAutoencoder,
    ...
    for layer_name in layer_order:
        ...
        emb_train = features_train[layer_name].to(device)
        emb_test = features_test[layer_name].to(device)
        
        color_acc = cross_nearest_neighbor_accuracy(
            emb_query=emb_test,
            labels_query=colors_test.to(device),
            emb_ref=emb_train,
            labels_ref=colors_train.to(device)
        )
        color_accs.append(color_acc)

        shape_acc = cross_nearest_neighbor_accuracy(
            emb_query=emb_test,
            labels_query=shapes_test.to(device),
            emb_ref=emb_train,
            labels_ref=shapes_train.to(device)
        )
        shape_accs.append(shape_acc)
        
        print(f"Layer: {layer_name:8s} | Color Acc: {color_acc:.2f}% | Shape Acc: {shape_acc:.2f}%")
        
        #### Your job 3 ends here ####

    # ---- plot figure like the reference ----
    plot_layerwise_nn(color_accs, shape_accs,
                      out_path=os.path.join(output_dir, "layerwise_nn_acc_tuned.png"))
